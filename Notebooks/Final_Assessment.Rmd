...
inf_HH %>% select(ref_date, age_group, amount_case) %>%
  ...
  ggplot(aes(x = ref_date, y = case_totals_by_age_group, fill = age_group)) +
  geom_area()
```
by `rep_date`:
```{r}
...
By `ref_date`:
```{r}
inf_HH %>% select(ref_date, sex, amount_case) %>%
    group_by(sex) %>%
    arrange(ref_date) %>%
    mutate(case_totals_by_sex = cumsum(amount_case)) %>%
    ggplot(aes(x = ref_date, y = case_totals_by_sex, fill = sex)) +
    geom_area()
```
By `rep_date`:
```{r}
inf_HH %>% select(rep_date, sex, amount_case) %>%
    group_by(sex) %>%
    arrange(rep_date) %>%
    mutate(case_totals_by_sex = cumsum(amount_case)) %>%
    ggplot(aes(x = rep_date, y = case_totals_by_sex, fill = sex)) +
    geom_area()
```
